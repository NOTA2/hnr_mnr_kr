...
- `0x17C1E4` 부근에 `0x083Dxxxx + length` 형식의 청크 디렉터리 후보가 존재한다.
- 이 디렉터리는 `0x3D1CE0..0x3D2557`, `0x3D2D60..0x3D3420`, `0x3D3420..0x3D6277` 같은 리소스 청크 경계를 설명해 준다.

## 다음 우선순위

1. `0x17C1E4` 청크 디렉터리를 누가 참조하는지 확인한다.
2. `0x3D2036` / `0x3D327E` 뱅크가 실제로 어떤 인덱스/구조체로 접근되는지 확인한다.
