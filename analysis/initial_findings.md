...
- `0x03005FF8` 은 독립 effect state 가 아니라 world-map 선택/hover location index byte 로 보는 해석이 가장 강하다.
- direct code slice 기준 확인된 writer 는 `0x06A52E` 하나이며, hit-test loop index `0..9` 를 저장한다.
- `0x06CEC0` 은 이 selected index 를 current-location byte `0x03006020` 으로 복사해 transition 좌표 계산을 시작한다.
...
1. `0x184A0C` effect/overlay row 의 `word0` / `word3` 세부 의미를 확인한다.
2. `0x1849A0` handler table singular cluster slot 소비 경로를 찾는다.
