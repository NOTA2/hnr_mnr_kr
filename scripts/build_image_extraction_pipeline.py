...
def build() -> dict:
    ...
    return {
        ...
        "pipeline_steps": [
            {
                "step": 1,
                "name": "대표 화면 확보",
                "description": "각 review unit 별로 실제 게임 화면 스크린샷 또는 참고 이미지를 최소 대표 케이스로 모아, 무엇을 찾아야 하는지 고정한다. 모든 화면을 수작업 수집하는 방식으로 진행하지 않는다.",
            },
            {
                "step": 2,
                "name": "영문판/일판 ROM 차이 확인",
                "description": "영문 패치 ROM 이 같은 화면을 수정했다면 같은 오프셋 LZ77 diff, 확장 영역 LZ77 후보, raw/tile 후보를 먼저 비교한다.",
            },
            {
                "step": 3,
                "name": "후보 자산 탐색",
                "description": "LZ77 scan, 4bpp dump, raw tile dump 를 쓰되, 결과가 보이지 않으면 실패가 아니라 tilemap/palette 분리 가능성으로 본다.",
            },
            {
                "step": 4,
                "name": "런타임 타일맵 추적",
                "description": "후보가 contact sheet 에 드러나지 않는 화면은 VRAM/tilemap/palette/OAM 조합으로 재구성되는 자산으로 보고 런타임 덤프나 스크린샷 기반 역추적을 수행한다.",
            },
            {
                "step": 5,
                "name": "덤프 시각 비교",
        ...
        "note": "이 문서는 실제 이미지 교체를 완료했다는 뜻이 아니라, review unit 별 추출/덤프/정리 루프를 바로 시작할 수 있게 scaffold 를 준비한 것이다. 타이틀처럼 contact sheet 에 바로 보이지 않는 그래픽은 런타임 tilemap-aware 추적으로 승격한다.",
    }
