#!/usr/bin/env python3

from __future__ import annotations

import json
from datetime import date
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
DATA_ROOT = ROOT / "confirmed_data"
OUT_DIR = DATA_ROOT / "image_inventory"
OUT_JSON = OUT_DIR / "image_text_inventory.json"
OUT_MD = OUT_DIR / "image_text_inventory.md"
OUT_README = OUT_DIR / "README.md"


def build() -> dict:
    return {
        "version": 1,
        "last_updated": date.today().isoformat(),
        "status": "in_progress",
        "note": "이 레이어는 inventory / 상태 관리용이며, 이미지에 구워진 텍스트 추출이 완료됐다는 뜻은 아니다.",
        "known_non_localizable_image_candidates": [
            {
                "offset": "0x00534874",
                "workspace": "confirmed_data/image_inventory/workspaces/07_ui_icon_badge_wordmarks",
                "preview": "confirmed_data/image_inventory/workspaces/07_ui_icon_badge_wordmarks/png_exports/004_off_00534874.png",
                "classification": "battle_hud_small_font_or_glyph_sheet",
                "localization_needed": False,
                "reason": "전투 HUD 의 작은 이름/수치 글자와 같은 계열로 보이는 원본 소형 폰트/글리프 시트 후보이다. 화면에 구워진 UI 라벨이나 문장 이미지가 아니므로 이미지 교체 대상은 아니다. 위쪽 깨짐은 타일맵/팔레트 없이 4bpp 타일을 일렬로 펼친 덤프 특성으로 본다.",
            }
        ],
        "confirmed_non_image_text_contexts": [
            {
                "context": "startup_intro_card",
                "reason": "startup_intro_texts.json 안의 고정 슬롯 텍스트 record 로 처리되며, 이미지에 구워진 텍스트가 아니다.",
            },
            {
                "context": "dialogue_box_payloads",
                "reason": "Entry8 과 Registry D 대사 문자열은 script record 로 추출되어 있고, portrait 는 별도 이미지 자산이다.",
            },
            {
                "context": "world_map_location_labels",
                "reason": "지역명은 location_texts.json 으로 추출되어 있고, 월드맵 텍스트 경로로 렌더링된다.",
            },
            {
                "context": "save_menu_prompts",
                "reason": "save/menu 문구는 01 FF command-stream record 로 추출되어 있으며, 이미지에 구워진 라벨이 아니다.",
            },
            {
                "context": "credits_strings",
                "reason": "크레딧은 패딩이 있는 일반 텍스트 record 로 추출되어 있으며, 이미지 텍스트로 확인되지 않았다.",
            },
        ],
        "review_units": [
            {
                "id": "title_logo_wordmark",
                "localization_needed": True,
                "status": "runtime_case_registered",
                "review_order": 1,
                "priority": "high",
                "review_goal": "타이틀 로고와 부제/워드마크가 텍스트 추출 밖의 이미지 자산인지 확인하고, 이후 교체 단위를 분리한다.",
                "recommended_probe_method": "타이틀 화면 런타임 케이스 등록 -> 영문판/일판 LZ77 diff 확인 -> 후보가 안 보이면 VRAM/tilemap/palette 역추적",
                "future_workspace": "confirmed_data/image_inventory/workspaces/01_title_logo_wordmark",
                "expected_text_kind": "메인 로고, 부제, 게임명 워드마크",
                "subunits": [
                    {
                        "id": "main_title_logo",
                        "label": "메인 타이틀 로고",
                        "status": "pending_review",
                        "first_action": "타이틀 화면 대표 스크린샷과 대조할 메인 로고 후보를 찾는다.",
                    },
                    {
                        "id": "subtitle_or_series_mark",
                        "label": "부제/시리즈 표기",
                        "status": "pending_review",
                        "first_action": "메인 로고와 별도 자산인지, 같은 시트 안의 일부인지 먼저 구분한다.",
                    },
                ],
                "notes": [
                    "메인 타이틀/로고 계열 텍스트는 현재 기준 추출 source 로 매핑되지 않는다.",
                    "사용자 제공 런타임 스크린샷에서 일본어 노출이 확인됐으므로 한글화 필요 케이스로 관리한다.",
                    "막연한 bucket 이 아니라 구체적인 검토 단위로 다룬다.",
                ],
            },
            {
                "id": "title_screen_static_menu_wordmarks",
                "localization_needed": True,
                "status": "runtime_case_registered",
                "review_order": 2,
                "priority": "high",
                "review_goal": "타이틀 화면의 새 게임/이어하기/통신 등 고정 워드마크가 이미지인지 확인한다.",
                "recommended_probe_method": "타이틀 화면 런타임 케이스 등록 -> 영문판/일판 LZ77 diff 확인 -> 후보가 안 보이면 VRAM/tilemap/palette 역추적",
                "future_workspace": "confirmed_data/image_inventory/workspaces/02_title_screen_static_menu_wordmarks",
                "expected_text_kind": "타이틀 메뉴 라벨, 모드 선택 라벨",
                "subunits": [
                    {
                        "id": "title_primary_menu_labels",
                        "label": "타이틀 기본 메뉴 라벨",
                        "status": "pending_review",
                        "first_action": "새 게임/이어하기 계열 고정 라벨 후보를 묶는다.",
                    },
                    {
                        "id": "title_mode_wordmarks",
                        "label": "통신/부가 모드 워드마크",
                        "status": "pending_review",
                        "first_action": "기본 메뉴와 다른 시트인지 먼저 확인한다.",
                    },
                ],
                "notes": [
                    "타이틀 화면의 고정 메뉴 라벨과 모드 워드마크는 이미지 기반일 가능성이 있다.",
                    "사용자 제공 런타임 스크린샷에서 `つづきから`, `はじめから`, `通信` 노출이 확인됐다.",
                    "현재 기준 추출 source 중 타이틀 화면 워드마크 아트를 직접 다루는 것은 없다.",
                ],
            },
            {
                "id": "event_or_cutscene_text_cards",
                "localization_needed": False,
                "status": "pending_review",
                "review_order": 3,
                "priority": "high",
                "review_goal": "이벤트 카드/장면 카드/챕터 카드처럼 화면 전체에 뜨는 이미지 텍스트를 우선 분류한다.",
                "recommended_probe_method": "대표 장면 스크린샷 수집 -> 카드성 오버레이 후보 LZ77 블록 탐색 -> 덤프 비교",
                "future_workspace": "confirmed_data/image_inventory/workspaces/03_event_or_cutscene_text_cards",
                "expected_text_kind": "장면 카드, 챕터 카드, 오버레이 캡션",
                "subunits": [
                    {
                        "id": "chapter_or_location_cards",
                        "label": "챕터/지역 카드",
                        "status": "pending_review",
                        "first_action": "고정형 카드 레이아웃이 있는지부터 구분한다.",
                    },
                    {
                        "id": "dramatic_overlay_captions",
                        "label": "연출용 오버레이 자막",
                        "status": "pending_review",
                        "first_action": "카드형과 즉시 분리해 후속 교체 전략을 나눈다.",
                    },
                ],
                "notes": [
                    "스토리/이벤트 카드나 오버레이는 일반 이벤트 일러스트보다 우선순위가 높은 이미지 텍스트 후보이다.",
                    "이미 비-이미지 텍스트로 확인된 대사 payload 와는 분리해서 본다.",
                ],
            },
            {
                "id": "dialogue_window_frame_art",
                "localization_needed": False,
                "status": "review_started",
                "review_order": 4,
                "priority": "medium",
                "review_goal": "대사창 프레임 안에 구워진 라벨이나 이름판이 있는지 먼저 분리한다.",
                "recommended_probe_method": "대사창 스크린샷 -> 프레임/판넬 후보 아트 덤프 -> 텍스트 유무 확인",
                "future_workspace": "confirmed_data/image_inventory/workspaces/04_dialogue_window_frame_art",
                "expected_text_kind": "이름판, 대사창 장식 라벨",
                "subunits": [
                    {
                        "id": "nameplate_or_label_panels",
                        "label": "이름판/장식 라벨 판넬",
                        "status": "pending_review",
                        "first_action": "텍스트가 들어가는 판넬이 독립 자산인지 확인한다.",
                    },
                    {
                        "id": "frame_only_art",
                        "label": "순수 프레임 아트",
                        "status": "review_started",
                        "first_action": "텍스트가 없으면 이미지 교체 우선순위를 낮춘다.",
                    },
                ],
                "notes": [
                    "대사 텍스트 자체는 추출 텍스트지만, 창 프레임 아트는 별도 시각 자산 family 이다.",
                    "구워진 텍스트가 없더라도 로컬라이징 QA 에서는 중요한 검토 단위다.",
                ],
            },
            {
                "id": "portrait_headshot_assets",
                "localization_needed": False,
                "status": "review_started",
                "review_order": 5,
                "priority": "medium",
                "review_goal": "화상/초상화 자산군을 대사 payload 와 분리해 나중에 이미지 작업 범위를 줄인다.",
                "recommended_probe_method": "대표 대사 장면 스크린샷 -> portrait 후보 아트 덤프 -> 동일 인물 변형 수집",
                "future_workspace": "confirmed_data/image_inventory/workspaces/05_portrait_headshot_assets",
                "expected_text_kind": "직접 텍스트보다는 화상 자산 분류",
                "subunits": [
                    {
                        "id": "main_party_portraits",
                        "label": "주요 캐릭터 화상",
                        "status": "review_started",
                        "first_action": "에드/알 등 대표 캐릭터 화상 변형 수를 대략 센다.",
                    },
                    {
                        "id": "npc_portraits",
                        "label": "NPC 화상",
                        "status": "pending_review",
                        "first_action": "NPC가 별도 시트인지 합쳐져 있는지 확인한다.",
                    },
                ],
                "notes": [
                    "portrait 는 이미지 자산이 맞다.",
                    "화자 텍스트 자체가 portrait 안에 구워져 있지는 않지만, 대사 QA 와 이후 이미지 작업에는 중요하다.",
                ],
            },
            {
                "id": "ui_panel_label_art",
                "localization_needed": False,
                "status": "pending_review",
                "review_order": 6,
                "priority": "medium",
                "review_goal": "패널 제목/탭/상태판처럼 이미지 라벨일 가능성이 높은 UI 자산을 먼저 묶는다.",
                "recommended_probe_method": "메뉴/인벤토리/상태창 스크린샷 -> 패널 후보 시트 덤프 -> 텍스트 여부 확인",
                "future_workspace": "confirmed_data/image_inventory/workspaces/06_ui_panel_label_art",
                "expected_text_kind": "패널 제목, 탭 라벨, 상단 헤더",
                "subunits": [
                    {
                        "id": "menu_tab_labels",
                        "label": "메뉴 탭 라벨",
                        "status": "pending_review",
                        "first_action": "탭이 텍스트 렌더링인지 이미지인지 먼저 판별한다.",
                    },
                    {
                        "id": "panel_header_wordmarks",
                        "label": "패널 헤더 워드마크",
                        "status": "pending_review",
                        "first_action": "저장/옵션/상태창 헤더가 있는지 확인한다.",
                    },
                ],
                "notes": [
                    "패널, 탭, 프레임형 UI 라벨 중 이미지에 글자가 구워졌을 수 있는 요소는 별도 단위로 검토한다.",
                    "일반 추출 save/menu/system 문자열과는 분리해서 본다.",
                ],
            },
            {
                "id": "ui_icon_badge_wordmarks",
                "localization_needed": False,
                "status": "pending_review",
                "review_order": 7,
                "priority": "low",
                "review_goal": "작은 아이콘/배지성 워드마크가 있다면 나중에 교체 난도가 높은 단위로 따로 뺀다.",
                "recommended_probe_method": "대표 UI 캡처 -> 작은 4bpp 배지 후보 덤프 -> 문자인지 아이콘인지 구분",
                "future_workspace": "confirmed_data/image_inventory/workspaces/07_ui_icon_badge_wordmarks",
                "expected_text_kind": "작은 워드마크, 배지, 미니 라벨",
                "subunits": [
                    {
                        "id": "mini_badge_labels",
                        "label": "미니 배지 라벨",
                        "status": "pending_review",
                        "first_action": "텍스트 기반인지 장식 아이콘인지 먼저 판별한다.",
                    }
                ],
                "notes": [
                    "아이콘/배지 크기의 워드마크는 큰 UI 패널과 따로 검토한다.",
                    "교체 전략이 패널 아트와 다를 가능성이 커서 분리 유지가 유용하다.",
                ],
            },
            {
                "id": "battle_result_or_reward_banners",
                "localization_needed": False,
                "status": "pending_review",
                "review_order": 8,
                "priority": "medium",
                "review_goal": "전투 결과/획득/보상 배너처럼 짧고 강한 워드마크 자산을 분리한다.",
                "recommended_probe_method": "전투 결과 화면 캡처 -> 배너 후보 LZ77/4bpp 덤프 -> 고정 워드마크 여부 확인",
                "future_workspace": "confirmed_data/image_inventory/workspaces/08_battle_result_or_reward_banners",
                "expected_text_kind": "승리/획득/보상 배너",
                "subunits": [
                    {
                        "id": "battle_result_banners",
                        "label": "전투 결과 배너",
                        "status": "pending_review",
                        "first_action": "결과 화면의 큰 워드마크를 우선 찾는다.",
                    },
                    {
                        "id": "reward_get_banners",
                        "label": "획득/보상 배너",
                        "status": "pending_review",
                        "first_action": "결과 배너와 다른 시트를 쓰는지 먼저 구분한다.",
                    },
                ],
                "notes": [
                    "결과/보상 배너는 전투나 전투 후 연출에서 이미지 기반 텍스트 후보가 될 수 있다.",
                    "이미 추출 텍스트인 대사와 용어 설명 source 와는 분리해서 본다.",
                ],
            },
        ],
        "operational_reading": [
            "이미지 텍스트 inventory 는 이제 별도 감사 트랙으로 시작된 상태다.",
            "정적 contact sheet 에서 보이지 않는다고 이미지 한글화 대상이 없다고 판정하지 않는다.",
            "타이틀/메뉴처럼 실제 화면에서 확인된 케이스는 runtime_visual_asset_audit 에 등록하고 tilemap-aware 추적으로 처리한다.",
            "몇몇 중요한 텍스트 문맥은 이미 비-이미지 텍스트로 확인됐으므로 번역 진행을 막지 않는다.",
            "남은 일은 기준 추출을 다시 여는 것이 아니라, 구체적인 review_unit 기준으로 자산을 검토하는 것이다."
        ],
        "next_actions": [
            "우선순위 1~2 review unit 인 타이틀 로고/메뉴부터 영문판 diff 와 런타임 tilemap 추적을 진행한다.",
            "review unit 별 workspace 아래에 candidate / dump / export / note 산출물을 모은다.",
            "image extraction pipeline 문서와 함께 실제 추출 단계로 이어간다.",
        ],
    }


def render_md(data: dict) -> str:
    lines = [
        "# 이미지 텍스트 inventory",
        "",
        f"- 상태: `{data['status']}`",
        f"- 마지막 갱신: `{data['last_updated']}`",
        "",
        "## 한글화 대상이 아닌 이미지 후보",
        "",
    ]
    for item in data.get("known_non_localizable_image_candidates", []):
        lines.append(
            f"- `{item['offset']}`: {item['classification']} / 한글화 대상 아님 / {item['reason']}"
        )
    lines.extend(["", "## 비-이미지 텍스트로 확인된 문맥", ""])
    for item in data["confirmed_non_image_text_contexts"]:
        lines.append(f"- `{item['context']}`: {item['reason']}")
    lines.extend(["", "## 검토 단위", ""])
    for item in data["review_units"]:
        lines.append(f"- `{item['id']}` (`{item['status']}`, `순서={item['review_order']}`, `우선순위={item['priority']}`)")
        lines.append(f"  - 목표: {item['review_goal']}")
        lines.append(f"  - 탐색 방식: {item['recommended_probe_method']}")
        lines.append(f"  - 예상 텍스트 종류: {item['expected_text_kind']}")
        lines.append(f"  - 작업 폴더: `{item['future_workspace']}`")
        lines.append("  - 세부 단위:")
        for subunit in item["subunits"]:
            lines.append(f"    - `{subunit['id']}`: {subunit['label']} / {subunit['status']} / 첫 행동: {subunit['first_action']}")
        for note in item["notes"]:
            lines.append(f"  - 참고: {note}")
    lines.extend(["", "## 현재 해석", ""])
    for note in data["operational_reading"]:
        lines.append(f"- {note}")
    lines.extend(["", "## 바로 다음 행동", ""])
    for note in data["next_actions"]:
        lines.append(f"- {note}")
    lines.append("")
    return "\n".join(lines)


def render_readme() -> str:
    return "\n".join(
        [
            "# 이미지 검토 인벤토리",
            "",
            "이 폴더는 **이미지에 구워진 텍스트 inventory / 이미지 자산 검토 상태**를 모아두는 곳이다.",
            "",
            "## 현재 파일",
            "",
            "- `image_text_inventory.json`",
            "- `image_text_inventory.md`",
            "- `image_extraction_pipeline.json`",
            "- `image_extraction_pipeline.md`",
            "- `workspaces/`",
            "",
            "## 사용 원칙",
            "",
            "- 기준 추출 텍스트와 이미지에 구워진 텍스트는 분리해서 관리한다.",
            "- 이 inventory 는 막연한 bucket 이 아니라 실제 검토 단위에 가까운 `review_units` 중심으로 유지한다.",
            "- 각 review unit 은 이후 추출 산출물을 모을 전용 workspace 를 갖는다.",
            "",
            "## 재생성",
            "",
            "```bash",
            "python3 scripts/build_image_text_inventory.py",
            "python3 scripts/build_image_extraction_pipeline.py",
            "```",
            "",
        ]
    )


def main() -> int:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    data = build()
    OUT_JSON.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    OUT_MD.write_text(render_md(data), encoding="utf-8")
    OUT_README.write_text(render_readme(), encoding="utf-8")
    print(f"wrote {OUT_JSON}")
    print(f"wrote {OUT_MD}")
    print(f"wrote {OUT_README}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
