...
def normalize_rgba_tile_to_binary(tile: Image.Image) -> bytes:
    ...
    for y in range(rgba.height):
        for x in range(rgba.width):
            r, g, b, a = pixels[x, y]
            # Treat non-transparent bright pixels as foreground. Output strict 0/34.
            if a >= 32 and max(r, g, b) >= 128:
                out.append(34)
            else:
                out.append(0)
    return bytes(out)
...
def main() -> int:
    ...
    for index, item in enumerate(manifest):
        ...
        pixels = normalize_rgba_tile_to_binary(tile)
