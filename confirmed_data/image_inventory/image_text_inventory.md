# 이미지 텍스트 inventory

- 상태: `in_progress`
- 마지막 갱신: `2026-05-18`

## 비-이미지 텍스트로 확인된 문맥

- `startup_intro_card`: startup_intro_texts.json 안의 고정 슬롯 텍스트 record 로 처리되며, 이미지에 구워진 텍스트가 아니다.
- `dialogue_box_payloads`: Entry8 과 Registry D 대사 문자열은 script record 로 추출되어 있고, portrait 는 별도 이미지 자산이다.
- `world_map_location_labels`: 지역명은 location_texts.json 으로 추출되어 있고, 월드맵 텍스트 경로로 렌더링된다.
- `save_menu_prompts`: save/menu 문구는 01 FF command-stream record 로 추출되어 있으며, 이미지에 구워진 라벨이 아니다.
- `credits_strings`: 크레딧은 패딩이 있는 일반 텍스트 record 로 추출되어 있으며, 이미지 텍스트로 확인되지 않았다.

## 검토 단위

- `title_logo_wordmark` (`pending_review`, `순서=1`, `우선순위=high`)
  - 목표: 타이틀 로고와 부제/워드마크가 텍스트 추출 밖의 이미지 자산인지 확인하고, 이후 교체 단위를 분리한다.
  - 탐색 방식: 타이틀 화면 진입 스크린샷 확보 -> LZ77 후보 스캔 -> 4bpp 덤프 비교
  - 예상 텍스트 종류: 메인 로고, 부제, 게임명 워드마크
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/01_title_logo_wordmark`
  - 세부 단위:
    - `main_title_logo`: 메인 타이틀 로고 / pending_review / 첫 행동: 타이틀 화면 대표 스크린샷과 대조할 메인 로고 후보를 찾는다.
    - `subtitle_or_series_mark`: 부제/시리즈 표기 / pending_review / 첫 행동: 메인 로고와 별도 자산인지, 같은 시트 안의 일부인지 먼저 구분한다.
  - 참고: 메인 타이틀/로고 계열 텍스트는 현재 기준 추출 source 로 매핑되지 않는다.
  - 참고: 막연한 bucket 이 아니라 구체적인 검토 단위로 다룬다.
- `title_screen_static_menu_wordmarks` (`pending_review`, `순서=2`, `우선순위=high`)
  - 목표: 타이틀 화면의 새 게임/이어하기/통신 등 고정 워드마크가 이미지인지 확인한다.
  - 탐색 방식: 타이틀 화면 캡처 -> 후보 타일 시트 덤프 -> 라벨별 분리 가능성 확인
  - 예상 텍스트 종류: 타이틀 메뉴 라벨, 모드 선택 라벨
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/02_title_screen_static_menu_wordmarks`
  - 세부 단위:
    - `title_primary_menu_labels`: 타이틀 기본 메뉴 라벨 / pending_review / 첫 행동: 새 게임/이어하기 계열 고정 라벨 후보를 묶는다.
    - `title_mode_wordmarks`: 통신/부가 모드 워드마크 / pending_review / 첫 행동: 기본 메뉴와 다른 시트인지 먼저 확인한다.
  - 참고: 타이틀 화면의 고정 메뉴 라벨과 모드 워드마크는 이미지 기반일 가능성이 있다.
  - 참고: 현재 기준 추출 source 중 타이틀 화면 워드마크 아트를 직접 다루는 것은 없다.
- `event_or_cutscene_text_cards` (`pending_review`, `순서=3`, `우선순위=high`)
  - 목표: 이벤트 카드/장면 카드/챕터 카드처럼 화면 전체에 뜨는 이미지 텍스트를 우선 분류한다.
  - 탐색 방식: 대표 장면 스크린샷 수집 -> 카드성 오버레이 후보 LZ77 블록 탐색 -> 덤프 비교
  - 예상 텍스트 종류: 장면 카드, 챕터 카드, 오버레이 캡션
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/03_event_or_cutscene_text_cards`
  - 세부 단위:
    - `chapter_or_location_cards`: 챕터/지역 카드 / pending_review / 첫 행동: 고정형 카드 레이아웃이 있는지부터 구분한다.
    - `dramatic_overlay_captions`: 연출용 오버레이 자막 / pending_review / 첫 행동: 카드형과 즉시 분리해 후속 교체 전략을 나눈다.
  - 참고: 스토리/이벤트 카드나 오버레이는 일반 이벤트 일러스트보다 우선순위가 높은 이미지 텍스트 후보이다.
  - 참고: 이미 비-이미지 텍스트로 확인된 대사 payload 와는 분리해서 본다.
- `dialogue_window_frame_art` (`review_started`, `순서=4`, `우선순위=medium`)
  - 목표: 대사창 프레임 안에 구워진 라벨이나 이름판이 있는지 먼저 분리한다.
  - 탐색 방식: 대사창 스크린샷 -> 프레임/판넬 후보 아트 덤프 -> 텍스트 유무 확인
  - 예상 텍스트 종류: 이름판, 대사창 장식 라벨
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/04_dialogue_window_frame_art`
  - 세부 단위:
    - `nameplate_or_label_panels`: 이름판/장식 라벨 판넬 / pending_review / 첫 행동: 텍스트가 들어가는 판넬이 독립 자산인지 확인한다.
    - `frame_only_art`: 순수 프레임 아트 / review_started / 첫 행동: 텍스트가 없으면 이미지 교체 우선순위를 낮춘다.
  - 참고: 대사 텍스트 자체는 추출 텍스트지만, 창 프레임 아트는 별도 시각 자산 family 이다.
  - 참고: 구워진 텍스트가 없더라도 로컬라이징 QA 에서는 중요한 검토 단위다.
- `portrait_headshot_assets` (`review_started`, `순서=5`, `우선순위=medium`)
  - 목표: 화상/초상화 자산군을 대사 payload 와 분리해 나중에 이미지 작업 범위를 줄인다.
  - 탐색 방식: 대표 대사 장면 스크린샷 -> portrait 후보 아트 덤프 -> 동일 인물 변형 수집
  - 예상 텍스트 종류: 직접 텍스트보다는 화상 자산 분류
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/05_portrait_headshot_assets`
  - 세부 단위:
    - `main_party_portraits`: 주요 캐릭터 화상 / review_started / 첫 행동: 에드/알 등 대표 캐릭터 화상 변형 수를 대략 센다.
    - `npc_portraits`: NPC 화상 / pending_review / 첫 행동: NPC가 별도 시트인지 합쳐져 있는지 확인한다.
  - 참고: portrait 는 이미지 자산이 맞다.
  - 참고: 화자 텍스트 자체가 portrait 안에 구워져 있지는 않지만, 대사 QA 와 이후 이미지 작업에는 중요하다.
- `ui_panel_label_art` (`pending_review`, `순서=6`, `우선순위=medium`)
  - 목표: 패널 제목/탭/상태판처럼 이미지 라벨일 가능성이 높은 UI 자산을 먼저 묶는다.
  - 탐색 방식: 메뉴/인벤토리/상태창 스크린샷 -> 패널 후보 시트 덤프 -> 텍스트 여부 확인
  - 예상 텍스트 종류: 패널 제목, 탭 라벨, 상단 헤더
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/06_ui_panel_label_art`
  - 세부 단위:
    - `menu_tab_labels`: 메뉴 탭 라벨 / pending_review / 첫 행동: 탭이 텍스트 렌더링인지 이미지인지 먼저 판별한다.
    - `panel_header_wordmarks`: 패널 헤더 워드마크 / pending_review / 첫 행동: 저장/옵션/상태창 헤더가 있는지 확인한다.
  - 참고: 패널, 탭, 프레임형 UI 라벨 중 이미지에 글자가 구워졌을 수 있는 요소는 별도 단위로 검토한다.
  - 참고: 일반 추출 save/menu/system 문자열과는 분리해서 본다.
- `ui_icon_badge_wordmarks` (`pending_review`, `순서=7`, `우선순위=low`)
  - 목표: 작은 아이콘/배지성 워드마크가 있다면 나중에 교체 난도가 높은 단위로 따로 뺀다.
  - 탐색 방식: 대표 UI 캡처 -> 작은 4bpp 배지 후보 덤프 -> 문자인지 아이콘인지 구분
  - 예상 텍스트 종류: 작은 워드마크, 배지, 미니 라벨
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/07_ui_icon_badge_wordmarks`
  - 세부 단위:
    - `mini_badge_labels`: 미니 배지 라벨 / pending_review / 첫 행동: 텍스트 기반인지 장식 아이콘인지 먼저 판별한다.
  - 참고: 아이콘/배지 크기의 워드마크는 큰 UI 패널과 따로 검토한다.
  - 참고: 교체 전략이 패널 아트와 다를 가능성이 커서 분리 유지가 유용하다.
- `battle_result_or_reward_banners` (`pending_review`, `순서=8`, `우선순위=medium`)
  - 목표: 전투 결과/획득/보상 배너처럼 짧고 강한 워드마크 자산을 분리한다.
  - 탐색 방식: 전투 결과 화면 캡처 -> 배너 후보 LZ77/4bpp 덤프 -> 고정 워드마크 여부 확인
  - 예상 텍스트 종류: 승리/획득/보상 배너
  - 작업 폴더: `confirmed_data/image_inventory/workspaces/08_battle_result_or_reward_banners`
  - 세부 단위:
    - `battle_result_banners`: 전투 결과 배너 / pending_review / 첫 행동: 결과 화면의 큰 워드마크를 우선 찾는다.
    - `reward_get_banners`: 획득/보상 배너 / pending_review / 첫 행동: 결과 배너와 다른 시트를 쓰는지 먼저 구분한다.
  - 참고: 결과/보상 배너는 전투나 전투 후 연출에서 이미지 기반 텍스트 후보가 될 수 있다.
  - 참고: 이미 추출 텍스트인 대사와 용어 설명 source 와는 분리해서 본다.

## 현재 해석

- 이미지 텍스트 inventory 는 이제 별도 감사 트랙으로 시작된 상태다.
- 몇몇 중요한 텍스트 문맥은 이미 비-이미지 텍스트로 확인됐으므로 번역 진행을 막지 않는다.
- 남은 일은 기준 추출을 다시 여는 것이 아니라, 구체적인 review_unit 기준으로 자산을 검토하는 것이다.
- 현재 active 추출 대상은 **실제로 한글화가 필요한 텍스트가 구워졌을 가능성이 높은 unit**만 포함한다.
- `dialogue_window_frame_art`, `portrait_headshot_assets` 는 시각 자산으로서 중요할 수는 있지만, 현재 단계에서는 **텍스트 이미지 교체 우선 대상이 아니므로 active 추출 목록에서 제외**한다.

## 바로 다음 행동

- 우선순위 1~3 review unit 부터 후보 자산 탐색용 스크린샷/덤프를 모은다.
- review unit 별 workspace 아래에 candidate / dump / export / note 산출물을 모은다.
- image extraction pipeline 문서와 함께 실제 추출 단계로 이어간다.
