...
11. 일본어/한자 자체를 제외한 원문의 특수기호, 문장부호, 번호 기호는 가능한 한 원문 형태를 유지한다. 숫자와 숫자 주변 슬래시는 원문 폭을 따른다. 원문이 `1/5`, `マッチョ1` 처럼 반각이면 번역도 반각이고, 원문이 `１／５`, `体力を５０` 처럼 전각이면 번역도 전각이다. `Registry D`, 코어 UI, 게임 용어 계열에서 이미 반각 공백/일부 기호가 안정적으로 쓰인 항목은 반각을 유지한다. `Entry8`, 이벤트 연출 텍스트, 오프닝/세이브/선택지 같은 counted/fixed 계열은 원문에 폭 단서가 없을 때 전각을 기본으로 한다. 인명/고유명사 내부의 일본식 중점 `・`는 한국어 번역에서 띄어쓰기로 바꾸고, `・・・` 말줄임표나 `１・２` 같은 숫자 구분 기호는 원문 기호를 유지한다.
...
- 반각 사용은 source family 별로 다르게 보되, **숫자와 숫자 주변 슬래시는 원문 폭을 따른다.** 원문이 `1/5`이면 번역도 `1/5`, 원문이 `１／５`이면 번역도 `１／５`로 맞춘다. `registry_d_fc_script_texts` 는 반각 공백/일부 기호 사용과 packed relocation 이 가능하다. `registry_a_entry8_prefixed_texts`, `inline_event_texts`, `startup_intro_texts`, `save_menu_texts`, `choice_yes_no_texts` 는 전각 우선 계열로 둔다.
...
- 이 family 에서는:
  ...
  - 2줄 설명창에서는 윗줄을 먼저 채운다. byte 와 첫 줄 표시 폭이 허용하면 앞줄을 우선 활용하되, fixed-slot 단문에서 사용자가 확정한 `창을연성해 공격` 같은 byte 민감 문구는 공백을 다시 넣지 않는다.
  - 앞줄 여백 활용은 실제 글자를 추가하는 정책이지 수동 padding 을 넣는 정책이 아니다. 최종 정렬 공백은 정규화/빌드 도구가 복원한다.
  - 단, 전투 ITEM 팝업의 회복약 설명(`체력 NN ...\x0b회복약`)은 별도 확인된 예외다. `0x0B` 는 줄바꿈이 아니라 뒷필드 separator 이며, `\n` 으로 바꾸면 텍스트가 잘못된 좌표에 찍힐 수 있다. 뒷필드 위치 보정과 잔상 제거용으로 `\x0b` 뒤 선행 공백 및 설명 끝 공백이 필요할 수 있으므로 GUI/정규화/빌드 경로는 이 계열의 뒤 필드 앞뒤 공백을 보존한다.
...
| 숫자 | 원문 숫자 폭을 따른다. 원문이 반각이면 반각, 원문이 전각이면 전각. 원문 숫자 의미와 번호 체계는 보존 |
| 영문 약어 | 전각 영문 사용. 예: `ＨＰ`, `ＡＳ`, `Ｒ 버튼` |
| `1/5`, `１／５` | 원문 슬래시/숫자 폭을 따른다. 원문이 `１／５`이면 번역도 `１／５`, 원문이 `1/5`이면 번역도 `1/5` |
