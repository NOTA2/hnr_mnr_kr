...
- `...` → `…`
- 공백 ` ` → 전각 공백 `　`
- 숫자 `0-9`는 일반 UI/대사/게임 용어에서는 반각 숫자를 유지한다.
- 단, `startup_intro_texts` 오프닝 첫 카드처럼 전용 렌더러가 반각 숫자를 안정적으로 표시하지 못하는 family 는 전각 숫자를 유지한다.
- `!` → `！`
- `?` → `？`
- `~` → `～`
- `(` → `（`
...
- 숫자는 family별 정책을 따른다. 일반 family 는 반각 숫자를 기본으로 유지하고, `startup_intro_texts` 는 전각 숫자를 유지한다. 영문자 자체는 반각 ASCII로 남기지 않는다.
