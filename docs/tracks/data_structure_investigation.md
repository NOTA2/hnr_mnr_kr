...
1. `0x076530` 포인터 허브를 참조하는 코드/구조체 확인
2. `0x17C1C0` 이 왜 공통 상위 레지스트리와 다른 레이아웃을 쓰는지 설명할 구조 찾기
3. 겹치는 엔트리의 관계를 부모/자식/메타데이터 관점에서 분류
4. `0x3D2036` 전투 기술 뱅크 참조 방식 확인
5. `0x3D327E` 대형 능력 뱅크 참조 방식 확인
6. 절대 포인터 외 인덱스/구조체/상대 오프셋 가능성 조사
